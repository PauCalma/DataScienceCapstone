...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        df['counts'] = df.groupby(['class'])['Launch Site'].transform('count')
        fig = px.pie(df, values='counts', 
        names='class', 
...
def get_scatter_chart(entered_site,entered_payload):
    filtered_df = spacex_df[(min(entered_payload)<=spacex_df['Payload Mass (kg)'])&(spacex_df['Payload Mass (kg)']<=max(entered_payload))]
    if entered_site == 'ALL':
